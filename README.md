...
游戏、动画、漫画、小说、电影、舞台剧和广播剧都可以研究。遇到不同改编、路线或版本时，报告会保留各自的边界，不会把互相冲突的材料混成一个结论。用户没有指定地区时，有独立国服版本的作品默认以国服已公开内容为主；外服只在进度、翻译、删改、脚本或视觉存在实质差异时补充。

剧情改写也不会被一句“版本不同”带过。报告会区分本地化差异、改编重写、脚本修订、追溯改写和矛盾未决。只有后续正典明确覆盖早期设定时才标记“追溯改写（俗称吃书）”；无法确认谁覆盖谁时，保留双方说法并标记“矛盾未决”。
...
- 作品、角色，以及使用哪个译名；
- 游戏路线、动画季度、漫画卷数等版本范围；
- 需要指定的服务器、地区或语言版本；未指定且存在国服时默认以国服为主；
- 是否允许剧透；
...
<details>
<summary>检查剧情改写或“吃书”</summary>

```text
$mrcarlsama-character-insight-canvas

研究这个角色时，请比较当前国服文本、旧版脚本和相关改编。
明确区分本地化差异、改编重写、脚本修订、追溯改写和矛盾未决。
不要把普通翻译差异或尚未解释的冲突直接写成“吃书”。
```
...
- **版本改动**：当材料确有差异时，直接展示改动类型、涉及版本、当前状态和两侧证据；
...
1. 确认角色、作品、地区版本、改编路线和剧透范围；
2. 收集材料，把事实、角色自述、视觉观察、研究者解释和版本改动分开记录；
3. 由独立子 Agent 核对关键论断、来源、不确定项和每一处版本改动；
